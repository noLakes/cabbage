  ...
  return {
    ...
    checkForm (edit = false) {
      ...
      save.addEventListener('click', () => {
        if (edit) {
          db.updateItem(name.dataset.uid, { name: name.value })
          container.nextSibling.querySelector('.name').innerHTML = name.value
          container.nextSibling.style.display = 'flex'
        } else {
          db.addCheck(document.querySelector('.modal-content').dataset.uid, name.value)
    ...
    check (check) {
      ...
      edit.addEventListener('click', () => {
        checkContainer.parentElement.insertBefore(this.checkForm(check), checkContainer)
    ...
    taskModal (task) {
      const modalContent = elements.basic('div', 'modal-content')
      modalContent.dataset.uid = task.uid
      if (task.complete) modalContent.classList.add('done')
      ...
      complete.addEventListener('click', (e) => {
        if (e.target.classList.contains('done')) {
          ...
          db.updateItem(task.uid, { complete: false })
        } else {
          e.target.classList.add('done')
          document.querySelector('.modal-content').classList.add('done')
          db.updateItem(task.uid, { complete: true })
        }
      })
      ...
      modalContent.appendChild(dueContainer)
      ...
      for (const key in task.children) {
      ...
      addItem.addEventListener('click', () => {
        checkList.appendChild(this.checkForm())
      })
